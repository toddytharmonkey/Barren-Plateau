...
if __name__ == "__main__":
    ...
    # colors generated by distincipy, hardcoded for 8 colors
    colors = [
        (0.0, 0.5, 1.0),
        (0.0, 1.0, 0.0),
        (0.19375703077229012, 0.4862621533963374, 0.05650337466880018),
        (0.4190812800557109, 0.13688197686030867, 0.5981929327756679),
        (0.5, 0.75, 0.5),
        (0.9444979490467592, 0.025931777171246506, 0.25769358848579005),
        (1.0, 0.0, 1.0),
        (1.0, 0.5, 0.0),
    ]
    ...
    for j, p in enumerate(probs):
        ...
        for i, n_qubits in enumerate(qubits):
            ...
            if p in [0, 0.05, 0.1] and n_qubits in [12, 14, 16]:
                p_i_m_given_thetas = np.load(
                    f"{n_qubits}_{p}_layeredresults_samples_nap_10000.npy"
                )
                mean, error = mutual_info_standard_error(p_i_m_given_thetas)
            elif p in [0.2, 0.3, 0.5] and n_qubits == 16:
                p_i_m_given_thetas = np.load(
                    f"{n_qubits}_{p}_layeredresults_samples_nap_10000.npy"
                )
                mean, error = mutual_info_standard_error(p_i_m_given_thetas)
                ...
                p_i_m_given_thetas = np.load(
                    f"{n_qubits}_{p}_layeredresults_samples_changeboth_1000.npy"
                )
                mean, error = mutual_info_standard_error(p_i_m_given_thetas)
            else:
                p_i_m_given_thetas = np.load(
                    f"{n_qubits}_{p}_layeredresults_samples_changeboth.npy"
                )
                mean, error = mutual_info_standard_error(p_i_m_given_thetas)
    ...
    plt.yscale("log")
    plt.legend()
    plt.savefig(f"qubits_at_layer_n.png")
    plt.clf()
