...
if __name__=="__main__": 
    ...
    zz_term = of.QubitOperator(f"Z{n_qubits-1} Z{0}", .5)
    ...
    exact_groundstate = torch.from_numpy(np.reshape(a,((2,)*n_qubits)).astype(np.complex64))

    #check that the groundstate is encoded directly in our arrays

    print("Groundstate value,", Inner(exact_groundstate, ApplyHam(exact_groundstate, "xxz_1_1_05", 8, True)))

    loaded_data = np.load("prob_0_HEA2_uber_parameters_q8_l7_shots1_postTrue_xxz_1_1_05_aware_thetas10_version1.2/all_run_info.npy", allow_pickle=True)
    ...
    variational_state = HEA_uber_gradient_by_layer(n_qubits, n_layers, parameters, gradient_technique="analytical",
                         ...
                         get_layered_results=False, ham_type="xxz_1_1_05",return_psi_list=True)[0]
    print("variational state shape", variational_state.shape)
    print(f"The fidelity between the variational state and the groundstate is {np.abs(Inner(exact_groundstate, variational_state))**2}")