...
for probability, measurements in tqdm(
    zip([0.1, 0.2], [point_one_measurement, point_two_measurement])
):
    ham_type = "z0z1"
    ...
    for run_i, output in tqdm(enumerate(all_run_data[2:])):
        print(np.abs(output[1] - (-1)))

        if np.abs(output[1] - (-1)) >= 0.0010:
            continue
